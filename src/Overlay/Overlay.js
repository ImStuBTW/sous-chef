...
import heyMp3 from './sounds/hey.mp3';
import listenMp3 from './sounds/listen.mp3';
...
class Overlay extends React.Component {
  componentDidMount() {
    ...
    function initNotice(msg) {
      ...
      if (msg.show) {
        let soundProp = whistleMp3;

        if(msg.sound === 'hey') { soundProp = heyMp3; }
        if(msg.sound === 'listen') { soundProp = listenMp3; }
    ...
    function handleTimerEmpty() {
      if (!probeAlert) {
        console.log('Overlay.js | handleTimerEmpty() | Timer empty, prove not alerting, stopping flashing.');
        stopBorderThrob();
      }
    }
    ...
    function addExpiredTimer() {
      expiredTimers++;
      console.log('Overlay.js | addExpiredTimer() | Adding timer, new value: ' + expiredTimers);
      startBorderThrob();
    }

    function removeExpiredTimer(msg) {
      console.log('Overlay.js | removeExpiredTimer() | Removing: ');
      console.log(msg);
      if(parseInt(msg.seconds) === 0) {
        expiredTimers--;
        console.log('Overlay.js | removeExpiredTimer() | Removing timer, new value: ' + expiredTimers);
        if (expiredTimers <= 0) {
          console.log('Overlay.js | removeExpiredTimer() | Stopping border flashing, timer value: ' + expiredTimers);
          stopBorderThrob();
