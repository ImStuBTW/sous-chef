...
module.exports = function(io, clientSocket) {
    ...
    const obsConnect = () => {
        ...
        .then(() => {
            console.log('obs.js | obs-connect | Successfully connected to OBS.');
            obsConnected = true;
            io.emit('obs-status', true);
            return obs.send('GetSceneList');
        })
        .then((data) => {
            console.log('obs.js | obs-connect | ' + data.scenes.length + ' available OBS scenes.');
            obsScenes = data.scenes.map((scene) => scene.name);
            console.log(`obs.js | obs-connect | Scene names ${JSON.stringify(obsScenes)}`);
    ...
    io.on('connection', (socket) => {
        ...
        fs.readFile('keyBindings.json', (err, data) => {
            if (err) {
                console.log('obs.js | readfile(\'keyBindings.js\') | Could not read saved OBS key bindings. Perhaps one doesn\'t exist yet?');
                console.error(err); 
            } else {
