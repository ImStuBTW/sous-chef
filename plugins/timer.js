...
module.exports = function(io) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('timer-fetch', function(fn) {
            ...
            console.log(`Sending timers '${timerJson}'.`);
        ...
        socket.on('timer-new', function(msg) {
            console.log(`New timer named '${msg.name}' w/${msg.seconds}secs`);
            ...
            let targetAchieved = (id) => {
                console.log(`Timer '${msg.name}' is done.`);
                io.emit('timer-expired', msg);
            };
        ...
        socket.on('timer-repeat', function(msg) {
            console.log(`Repeating timer '${msg.name}'.`);
        ...
        socket.on('timer-delete', function(msg) {
            console.log(`Deleting timer '${msg.name}' w/${msg.seconds}secs remaining`);
            ...
            if (streamTimers.length === 0) {
                console.log('No timers left, emitting event');
